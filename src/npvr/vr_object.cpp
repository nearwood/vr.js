...
bool VRObject::InvokeExec(const NPVariant* args, uint32_t arg_count, NPVariant* result)
{
	// arg0: command id
	// arg1: command string
	if (arg_count != 2) {
...
bool VRObject::InvokePoll(const NPVariant* args, uint32_t arg_count, NPVariant* result)
{
	std::ostringstream s;
...
bool VRObject::Invoke(NPIdentifier name, const NPVariant* args, uint32_t argCount, NPVariant* result)
{
	if (name == exec_id_) {
...
bool VRObject::InvokeDefault(const NPVariant* args, uint32_t argCount, NPVariant *result)
{
	return false;
}
